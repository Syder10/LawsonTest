  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ minWidth: 320 }}>
        ...
        {/* Color mappings: Total(#059669/emerald), Bitters(#000000/black), Ginger(#eab308/yellow) */}
        {filter === "all" ? <>{toPath(allSeries, "#059669")}{toPath(bitSeries, "#000000")}{toPath(ginSeries, "#eab308")}</> : filter === "Bitters" ? toPath(bitSeries, "#000000") : toPath(ginSeries, "#eab308")}
        {filter === "all" ? <>{toDots(allSeries, "#059669")}{toDots(bitSeries, "#000000")}{toDots(ginSeries, "#eab308")}</> : filter === "Bitters" ? toDots(bitSeries, "#000000") : toDots(ginSeries, "#eab308")}
        {filter === "all" && (
          <g>
            <rect x={PL} y={PT - 14} width={10} height={10} rx={2} fill="#059669" /><text x={PL + 13} y={PT - 5} fontSize="10" fill="#374151">Total</text>
            <rect x={PL + 50} y={PT - 14} width={10} height={10} rx={2} fill="#000000" /><text x={PL + 63} y={PT - 5} fontSize="10" fill="#374151">Bitters</text>
            <rect x={PL + 115} y={PT - 14} width={10} height={10} rx={2} fill="#eab308" /><text x={PL + 128} y={PT - 5} fontSize="10" fill="#374151">Ginger</text>
          </g>
        )}
      </svg>
    </div>
...
function StatCard({ label, value, sub, icon: Icon, sparkData, sparkColor, highlight = false, borderColorClass = "" }: {
  label: string; value: string | number; sub?: string; icon?: any
  sparkData?: number[]; sparkColor?: string; highlight?: boolean; borderColorClass?: string;
}) {
  return (
    <div className={`glass-panel p-5 space-y-2 hover:scale-[1.02] transition-all ${highlight ? "ring-2 ring-emerald-400/40" : ""} ${borderColorClass ? `border-l-4 ${borderColorClass}` : ""}`}>
  ...
  return (
    <div className="w-full min-h-screen bg-mesh p-3 md:p-6 mb-12 animate-fade-in-up">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Live Stocks Display Grid Feature */}
        <div className="glass-panel p-5 md:p-6 bg-white/90">
            <h2 className="text-sm md:text-base font-black text-emerald-950 uppercase tracking-widest mb-4">Live Inventory Trackers</h2>
            <LiveStocksDisplay />
        </div>
        ...
        {isLoadingKPIs ? (
          ...
          <div className="space-y-6">
            ...
            <div className="glass-panel p-5 md:p-6">
              <SectionHeader label="Historical Stock Analytics" />
              <div className={`grid gap-4 ${productFilter === "all" ? "grid-cols-2" : "grid-cols-1 max-w-xs"}`}>
                {(productFilter === "all" || productFilter === "Bitters") && (
                  <StatCard label="Available — Bitters" value={d.live_bitters_stock} sub="Cartons in stock" icon={Package} sparkData={trendBitters} sparkColor="#000000" borderColorClass="border-black" />
                )}
                {(productFilter === "all" || productFilter === "Ginger") && (
                  <StatCard label="Available — Ginger" value={d.live_ginger_stock} sub="Cartons in stock" icon={Package} sparkData={trendGinger} sparkColor="#eab308" borderColorClass="border-yellow-400" />
                )}
              </div>
            </div>
            ...
            <div className="glass-panel p-5 md:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                {(productFilter === "all" || productFilter === "Bitters") && <StatCard label="Labels — Bitters" value={d.labels_bitters_remaining} sub="Units" icon={Tag} borderColorClass="border-black" />}
                {(productFilter === "all" || productFilter === "Ginger") && <StatCard label="Labels — Ginger" value={d.labels_ginger_remaining} sub="Units" icon={Tag} borderColorClass="border-yellow-400" />}
                {(productFilter === "all" || productFilter === "Bitters") && <StatCard label="Caramel — Bitters" value={d.caramel_bitters_remaining} sub="Remaining" icon={FlaskConical} borderColorClass="border-black" />}
                {(productFilter === "all" || productFilter === "Ginger") && <StatCard label="Caramel — Ginger" value={d.caramel_ginger_remaining} sub="Remaining" icon={FlaskConical} borderColorClass="border-yellow-400" />}
            ...
            <div className="glass-panel p-5 md:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                {(productFilter === "all" || productFilter === "Bitters") && <StatCard label="Labels Used — Bitters" value={d.total_labels_bitters_used} sub="Units" icon={Tag} borderColorClass="border-black" />}
                {(productFilter === "all" || productFilter === "Ginger") && <StatCard label="Labels Used — Ginger" value={d.total_labels_ginger_used} sub="Units" icon={Tag} borderColorClass="border-yellow-400" />}
                {(productFilter === "all" || productFilter === "Bitters") && <StatCard label="Bottles Used — Bitters" value={bottlesBitters} sub={`${d.bitters_cartons.toLocaleString()} ctns × 12`} icon={Package} borderColorClass="border-black" />}
                {(productFilter === "all" || productFilter === "Ginger") && <StatCard label="Bottles Used — Ginger" value={bottlesGinger} sub={`${d.ginger_cartons.toLocaleString()} ctns × 12`} icon={Package} borderColorClass="border-yellow-400" />}
            ...
            <div className="glass-panel p-5 md:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ...
                {(productFilter === "all" || productFilter === "Bitters") && <StatCard label="Bitters Production" value={d.bitters_cartons} sub="Cartons" icon={Package} sparkData={trendBitters} sparkColor="#000000" borderColorClass="border-black" />}
                {(productFilter === "all" || productFilter === "Ginger") && <StatCard label="Ginger Production" value={d.ginger_cartons} sub="Cartons" icon={Package} sparkData={trendGinger} sparkColor="#eab308" borderColorClass="border-yellow-400" />}
              </div>
            ...
            {(productFilter === "all" || productFilter === "Bitters") && (
              <div className="border-4 border-black bg-black/5 rounded-xl p-5 md:p-6">
                <SectionHeader label="Production Inputs & Materials — Bitters" badge={d.bitters_cartons} badgeColor="bg-black text-white" />
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  <StatCard label="Alcohol for Bitters" value={d.alcohol_used_for_bitters_drums} sub="Drums" icon={Droplet} borderColorClass="border-black" />
                  <StatCard label="Concentrate Used" value={d.total_concentrate_used_litres} sub="Litres" borderColorClass="border-black" />
                  <StatCard label="Spices Used" value={d.total_spices_used_litres} sub="Litres" borderColorClass="border-black" />
                  <StatCard label="Caramel Used" value={d.total_caramel_used_gallons} sub="Gallons" borderColorClass="border-black" />
                  <StatCard label="Water Used" value={d.total_water_litres} sub="Litres" borderColorClass="border-black" />
                  <StatCard label="Labels Used" value={d.total_labels_bitters_used} sub="Units" icon={Tag} borderColorClass="border-black" />
                  <StatCard label="Caps Used" value={d.total_caps_used} sub="Units" icon={Box} borderColorClass="border-black" />
                  <StatCard label="Bottles Used" value={bottlesBitters} sub={`${d.bitters_cartons.toLocaleString()} × 12`} icon={Package} borderColorClass="border-black" />
                </div>
              </div>
            )}
            ...
            {(productFilter === "all" || productFilter === "Ginger") && (
              <div className="border-4 border-yellow-400 bg-yellow-50 rounded-xl p-5 md:p-6 mt-6">
                <SectionHeader label="Production Inputs & Materials — Ginger" badge={d.ginger_cartons} badgeColor="bg-yellow-400 text-black" />
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  <StatCard label="Alcohol for Ginger" value={d.alcohol_used_for_ginger_drums} sub="Drums" icon={Droplet} borderColorClass="border-yellow-400" />
                  <StatCard label="Water Used" value={d.ginger_water_used_litres} sub="Litres" borderColorClass="border-yellow-400" />
                  <StatCard label="G/T Juice" value={d.ginger_gt_juice_litres} sub="Litres" borderColorClass="border-yellow-400" />
                  <StatCard label="Spices Used" value={d.ginger_spices_used_litres} sub="Litres" borderColorClass="border-yellow-400" />
                  <StatCard label="Caramel Used" value={d.ginger_caramel_used_gallons} sub="Gallons" borderColorClass="border-yellow-400" />
                  <StatCard label="Labels Used" value={d.total_labels_ginger_used} sub="Units" icon={Tag} borderColorClass="border-yellow-400" />
                  <StatCard label="Bottles Used" value={bottlesGinger} sub={`${d.ginger_cartons.toLocaleString()} × 12`} icon={Package} borderColorClass="border-yellow-400" />
                </div>
              </div>
            )}
