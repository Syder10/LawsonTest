...
function Sparkline({
  ...
  data: number[]; color: string; gradId: string; width?: number; height?: number
}) {
...
function Tile({
  ...
  const styles: Record<string, string> = {
    white: "bg-white border border-slate-200", bitters: "bg-[#111827]",
    ginger: "bg-amber-500", slate: "bg-slate-100", "amber-soft": "bg-amber-50 border border-amber-100",
  }
...
function TrendChart({
  ...
  data: KPIData["monthly_trend"]; filter: Product
  isMonthly: boolean; selMonth: number; selYear: number
}) {
  ...
  const W = 800, H = 236, PL = 60, PR = 20, PT = 30, PB = 56
...
export function ManagerDashboard({ userId }: { userId: string }) {
  ...
  // ── KEY FIX: fetchKPIs reads state directly — no parameters ──
  ...
  // ── KEY FIX: re-fetch whenever period / month / year changes ──
  useEffect(() => {
    fetchKPIs()
  }, [fetchKPIs])

  // Auto-refresh every 5 min (uses latest fetchKPIs)
  useEffect(() => {
    ...
    <>
      ...
      <div className="dash w-full bg-[#f7f7f5] min-h-screen -m-4 sm:-m-6 md:-m-10 px-4 sm:px-6 md:px-10 py-8">
        <div className="max-w-[1400px] mx-auto space-y-6">
          ...
          {/* ══ FILTERS — just set state, useEffect auto-fetches ══ */}
